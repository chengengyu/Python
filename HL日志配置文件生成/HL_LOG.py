...
filedir = r'D:\ViewRoot\chengengyu_view_FddRrm\EMB5216_HL\RRM'
filelist = []
filename_file = open('log_filename.txt','w')
...
logline = ''
log_list = []
log_check_result = open('log_check_result.txt','w')
    ...
    try:
        ...
        for num,eachline in enumerate(fp):
            try:
                ...
                #判断是否是L2日志记录行
                if re.search(r'L3.Log.No', eachline):
                    logline = eachline.strip()
                #考虑可能多行的情况，将每一行串起来
                elif logline:
                    logline = logline + eachline.strip()
                #HL日志记录以/* L3.Log.No...*/的形成存在，如果读取完整则添加至list中，否则继续读取下一行。
                if re.search(r'/\* *L3.Log.No.+?\*/', logline):
                    log_list.append(logline)
                    logline = ''
            except:
                #解析失败，通常是因为一些汉字的半角全角有问题
                err = "无法解析的行: 文件为" + file + "行号为:" + str(num+1) + '\n'
                log_check_result.write(err)
        fp.close
    except:
        err = '无法打开文件' + file +'\n'
        log_check_result.write(err)

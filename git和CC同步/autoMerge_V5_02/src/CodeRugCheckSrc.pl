...
######2010/08/31   for (     ;
######2010/08/31  检查模式下对“mac\inc”的误检
######2010/08/31  宏定义多行的检查问题
######2010/09/02  (Uint)&start_cycle
######2010/09/13  全局常量的检查是否要求一致
######2010/09/13  去掉非CHECK_LIST检查项
######2010/09/26  由文件dir.pl生成本文件
######2010/09/26  去除非CheckList的检查项: 1. 括号紧随，分号冒号后空格等；2.强制类型转换；3.do while误判；4.文件路径名误判；5.全局常量；
######2011/02/17  取消行尾空格的检查
######2011/02/17  排除对第三方文件的规范性检查
######2011/02/17  修正for循环换行时对于“if/for/while/switch是否与语句在同一行”的误判
######2011/02/17  修正打印中不能出现NULL的问题；
######2011/02/18  修正return -1的情况
######2011/02/18  存在打印行对“单行多语句”的判断
######2011/02/28  代码中包含注释的行取消检查和修改
######2011/02/28  $macro_array[1]初始化
######2011/02/28  增加signed int的判断
######2011/04/13  取消Undocheckout时扩展名的判断
...
sub MacroBracket()
{
  ...
  if (($macro_line =~ m{define +[a-zA-Z0-9_]+\( *\b([a-zA-Z0-9_]+)\b})
      && ($macro_line !~ m{define +[a-zA-Z0-9_]+\([a-zA-z0-9,_ ]+\) +[a-zA-Z0-9_]+ *\(})
...
{ 
  ...
  if (($code_line =~ m{^ *(extern +)?(static +)?(const +)?(signed)?(unsigned)?(int|short|char|float|double|long) +\*?[a-zA-Z0-9_]+ *(;|\[|=)}))
  {
    print CCR_File  "  $Paramenter[0], $line_cnt行，禁止使用int、short、char、float、double、long定义变量:\n";
    print CCR_File  "$code_line\n";     
  }
...
{ 
  ...
  @get_third_party_files_dir = split(CodeRugCheckSrc, $0);
  ...
  $pfile_name=$file;
  $pfile_name=~s{(.+)(\/)([A-Za-z0-9_]+\.)([a-zA-Z0-9_]+)$}{$3$4}g;
...
@get_file_dir = split("CodeRugCheckSrc", $0);
